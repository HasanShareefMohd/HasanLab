...
public class TaxCalculator {
	class TaxCalculator{
		public double calculateTax(String empName,boolean isIndian,double empSal)   {
			double taxAmount = 0;
		try {//try block
			if(isIndian==false) {//if not Indian throw this exception
				throw new CountryNotValidException("The employee should be an Indian citizen for calculating tax”"); //custom exception
			}
			if(empName=="") { //If the value of name this exception will happen
				throw new EmployeeNameInvalidException("The employee name cannot be empty");//customexception
			}
		if(empSal>100000) {
		 return taxAmount =empSal *8/100;//logic of tax calculation
		}
		if(empSal>=50000 && empSal<100000) {
		 return taxAmount =empSal *6/100 ;//logic of tax calculation
		}
		if(empSal>=30000 && empSal<50000) {
			return taxAmount =empSal *5/100 ;//logic of tax calculation
		}
		if(empSal>=10000 && empSal<30000) {
			return taxAmount =empSal *4/100 ;//logic of tax calculation
		}
		else
	...
	public class CalculatorSimulator {
		public static void main(String[] args)   {//main method object of class TaxCalculator
